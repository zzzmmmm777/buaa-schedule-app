...
function _cacheLocal() {
  try { localStorage.setItem(TODO_KEY, JSON.stringify(_todos)); } catch {}
}
...
async function syncTodos() {
  try {
    ...
    if (!json || !Array.isArray(json.items)) throw new Error('bad_response');
    _todos = json.items.map(item => ({
      id: item.id,
      title: item.title,
      date: item.date,
      time: item.time || null,
...
async function addTodo(obj) {
  const json = await _apiPost('create', {
    title: obj.title,
    date: obj.date,
    time: obj.time || null,
    loc: obj.loc || null,
...
async function updateTodo(id, changes) {
  const json = await _apiPost('update', { id, ...changes });
  const item = json.item;
...
async function deleteTodo(id) {
  await _apiPost('delete', { id });
  _todos = _todos.filter(t => t.id !== id);
  _cacheLocal();
}

async function toggleTodo(id) {
  const json = await _apiPost('toggle', { id });
  const item = json.item;
  const idx = _todos.findIndex(t => t.id === id);
  if (idx >= 0) {
    _todos[idx].done = !!item.done;
    _cacheLocal();
    return _todos[idx];
  }
  return null;
...
function attachTodoEventListeners(container) {
  container.querySelectorAll('[data-toggle]').forEach(el => {
    el.addEventListener('click', async e => {
      e.stopPropagation();
      try {
        await toggleTodo(el.dataset.toggle);
      } catch {}
      renderTodoList();
      if (currentTab === 'home') renderHomeTodos(today());
    });
  });
